...
int main(int argc, char **argv)
{
	...
	//char* testText = (char*)malloc(sizeof(char)*7);
	char* testText = "\nggaba";
	//memset(testText, 'a', 7*sizeof(char));
	char* d_testText;
	cudaMalloc(&d_testText, 7*sizeof(char));
	cudaMemcpy(d_testText, testText, 7*sizeof(char), cudaMemcpyHostToDevice);
	CountPosition(d_testText, pos_yours_gpu, 7);
